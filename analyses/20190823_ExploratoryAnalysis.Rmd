...
Load libraries; Data is a Phased VCF file downloaded from `https://www.encodeproject.org/files/ENCFF241LKI/@@download/ENCFF241LKI.vcf.gz` which was retrieved by using `wget https://www.encodeproject.org/files/ENCFF241LKI/@@download/ENCFF241LKI.vcf.gz'` on 2019-08-20
...
To get an idea of the data, various parameters were plotted. For example, the amount of SNVs vs the amount of other variations (insertions, deletions, structural variants etc.).  

```{r Bargraphs, echo=FALSE, fig.width=5, warning=FALSE}
#Plot SNV vs other
...
```{r summarizing bargraph, echo = F, warning=F} 
...
b <- barplot(var[,2], las = 2, ylab = "Percentage", ylim = c(0,130), main = "All variants")
...
It can be seen that in the last karyoplot above there are certain regions that do not contain many black dots. This is because these regions have suffered from loss of heterozygosity (LOH). Which has also been reported for chromos
