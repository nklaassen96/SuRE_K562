...
above you read data and assign it to a variable with a specific name (`assign(name, tabsep)`). Next, you asve the data to a file with a particular name. Thus the intermediate variable holding the data doesn't need a particular name. You could do something like:
```{r, eval=F}
for (chrom in paste0("chr", c(1:22, "X"))){
  fname <- paste0(dir, chrom, ".txt.gz")
  tab <- fread(fname, ...)
  .
  tabsep <- ...
  .
  tabfinal <- ...
  .
  ofname <- paste0("/DATA/usr/n.klaassen/projects/SuRE_K562/data/processed/SuRE_Counts_Separated_20190917/SuRE-Counts_Separated_", chrom,  ".txt")
  write.table(tabfinal, ofname, ...)
}
```

Also, I would use a variable to hold the filename (`fname`) so that the statement `fread(fname, ...)` is more clear and is not overwhelmed by the long string specifying the filename
...
If I understand correctly above you make a table with some hardcoded results (the values `c(3910735, 5404995, 2838568, 32022...))`
This is not how you should do it :-)
If the input data changes your code will still show the same table. So the numbers should be calculated in the code
...
I think a more efficient way to test if both variants are in s1 uses "%in%" (test for both 1 and 2 whether they are present in your vector 'x') and "all" (are all elements in the vector TRUE):
```
tapply(s1$SNPvar, s1$SNP_ID, function(x){all(c(1,2) %in% x)})
```
